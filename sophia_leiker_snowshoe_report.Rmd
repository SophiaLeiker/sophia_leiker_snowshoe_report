...
hare_females_stats <- hare_weights %>% 
  filter(sex == "Female") %>% 
  group_by(grid) %>% 
  summarise(mean_female = mean(weight, na.rm = TRUE),
            median_female = median(weight, na.rm = TRUE),
            sd_female = sd(weight, na.rm = TRUE),
...
hare_males_stats <- hare_weights %>% 
  filter(sex == "Male") %>%
   group_by(grid) %>% 
  summarise(mean_male = mean(weight, na.rm = TRUE),
            median_male = median(weight, na.rm = TRUE),
            sd_male = sd(weight, na.rm = TRUE),
...
**Table 2.** Descriptive statistics (mean, median, standard deviation, min, and max) for juvenile female hare weights
```{r}
hare_females_stats %>% 
...
**Table 3.** Descriptive statistics (mean, median, standard deviation, min, and max) for juvenile male hare weights
```{r}
hare_males_stats %>% 
...
**Analysis** On average juvenile male hares have higher weights than juvenile female hares. The male mean is (`r round(male_mean,2)` (g) $\pm$ with a standard deviation of `r round(male_sd, 2)`. The female mean is `r round(female_mean,2)` $\pm$ with a standard deviation of `r round(female_sd, 2)` (g.) The absolute difference in means is `r round(male_mean, 2) - round(female_mean, 2)` (g) which equates to a `r round((male_mean - female_mean)/((male_mean + female_mean)/2)*100, 2)` % difference). On average, the juvenile male hares have higher weight values than juvenile female hares, the difference in means is significant as indicated by the p-value from the Welch Two Sample t-test is `r round(hare_test_tidy$p.value,3)` = `r round(hare_test_tidy$statistic,2)`, p < 0.001), and the effect size is large (Cohen's *d* = `r round(hare_effsize$estimate,2)`).
